...
int rbcp_open(char *hostname, int port)
{
...
int rbcp_send(int sock, char *buf, int len)
{
	int status;
...
int rbcp_receive(int sock, char *buf)
{
	int status;
	//socklen_t fromlen;
	...
	if (status < 0) {
		perror("rbcp_receive recvfrom (Timeout ?)");
		return -1;
	}
	return status;
}

int rbcp_close(int sock)
{
	return close(sock);
}
...
int rbcp_read(int sock, char *buf, unsigned int addr, int len)
{
	...
	status = rbcp_send(sock, (char *)&bcp, sizeof(bcp));
	if (status < 0) printf("rbcp_read udp_send: error\n");
	//fprintf(stderr, "#D send\n");
	rlen = rbcp_receive(sock, buf);
	...
	if (rlen > (int)(sizeof(struct rbcp_header))) {
		...
		fprintf(stderr, "rbcp_read: RBCP Read err.\n");
	}

	return rlen;
}

int rbcp_write(int sock, char *data, unsigned int addr, int len)
{
	struct bcp_header *bcp;
	struct rbcp_header *rbcp;
	char *bcp_body;
	...
	if (len < 2048 - sizeof(struct bcp_header)) {
		...
		fprintf(stderr, "rbcp_write: Buffer overflow %d\n", len);
		return -1;
	}

	status = rbcp_send(sock, (char *)&sbuf, sizeof(struct bcp_header) + len);
	if (status < 0) printf("rbcp_write udp_send: error\n");
	//fprintf(stderr, "#D send\n");
	rlen = rbcp_receive(sock, rbuf);
	//fprintf(stderr, "#D receive %d\n", rlen);
...
int main(int argc, char* argv[])
{
	...
	printf("host: %s\n", hostname);
	sock = rbcp_open(hostname, port);

	
	if (rlen > 0) {
		printf("read: 0x%x : %d\n", raddress, rlen);
		rbcp_read(sock, buf, raddress, rlen);
	}
	if (wdata >= 0) {
		printf("write: 0x%x : %x\n", waddress, wdata);
		buf[0] = wdata & 0xff;
		buf[1] = 0;
		status = rbcp_write(sock, buf, waddress, 1);
	...
	if (seq1 == 1) {
		...
		for (j = 0 ; j < 128 ; j++) {
			...
			buf[0] = j;
			rbcp_write(sock, buf, 0 + 128*j, 128);
		}
	}
